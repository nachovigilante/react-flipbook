...
export const calculateFlipping = (pageSize, draggingPosition) => {
    ...
    const foldedLength = clamp((Math.pow(h, 2) + Math.pow(x, 2)) / (2 * x), 0, pageSize.width);
    ...
    let foldedHeight = angle > 0
        ? clamp(virtualCorner.x / Math.cos(((90 - angle) * Math.PI) / 180), 0, pageSize.height)
        : clamp(virtualCorner.x / Math.cos(((90 + angle) * Math.PI) / 180), 0, pageSize.height);
    ...
    let topFoldedLength = clamp(topRightCornerOverlap / Math.sin((angle * Math.PI) / 180), 0, pageSize.width);
    if (angle < 0)
        topFoldedLength = 0;
    if (virtualCorner.y > pageSize.height) {
        foldedHeight = pageSize.height;
        topFoldedLength = clamp(-topRightCornerOverlap / Math.tan(((360 - angle) * Math.PI) / 180), 0, pageSize.width);
    }
