...
exports.calculateFlipping = exports.clamp = void 0;
const clamp = (num, min, max) => Math.min(Math.max(num, min), max);
exports.clamp = clamp;
const calculateFlipping = (pageSize, draggingPosition) => {
    ...
    const foldedLength = (0, exports.clamp)((Math.pow(h, 2) + Math.pow(x, 2)) / (2 * x), 0, pageSize.width);
    ...
    let foldedHeight = angle > 0
        ? (0, exports.clamp)(virtualCorner.x / Math.cos(((90 - angle) * Math.PI) / 180), 0, pageSize.height)
        : (0, exports.clamp)(virtualCorner.x / Math.cos(((90 + angle) * Math.PI) / 180), 0, pageSize.height);
    ...
    let topFoldedLength = (0, exports.clamp)(topRightCornerOverlap / Math.sin((angle * Math.PI) / 180), 0, pageSize.width);
    if (angle < 0)
        topFoldedLength = 0;
    if (virtualCorner.y > pageSize.height) {
        foldedHeight = pageSize.height;
        topFoldedLength = (0, exports.clamp)(-topRightCornerOverlap / Math.tan(((360 - angle) * Math.PI) / 180), 0, pageSize.width);
    }
    // console.log(topLeftCornerHeight, opposite, falopa);
    const correctedAngle = virtualCorner.y > pageSize.height || angle > 0 ? -angle : 180 - angle;
    return {
        foldedLength,
        foldedHeight,
        topFoldedLength,
...
exports.calculateFlipping = calculateFlipping;
